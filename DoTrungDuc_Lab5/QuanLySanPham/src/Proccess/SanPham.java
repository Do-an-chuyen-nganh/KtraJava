...
public class SanPham {
   public Connect cn= new Connect(); 
        ...
        String sql = "SELECT MaSP,TenSP,Dongia,L.Maloai, Tenloai FROM Sanpham S, LoaiSP L where L.Maloai=S.Maloai and MaSP='" + ma +"'"; return cn.LoadData(sql);
        }
        //Truy van du lieu trong Table Sanpham theo gia
        public ResultSet ShowSPTheogia(int giatu, int giaden) throws SQLException{
            if(cn == null)
                cn.connectSQL();
            String sql = "SELECT MaSP,TenSP,Dongia,L.Maloai FROM Sanpham S, LoaiSP L where L.Maloai=S.Maloai and Dongia between " + giatu +" and " + giaden; return cn.LoadData(sql);
        }
        public ResultSet ShowSPTheoten(String ten) throws SQLException{
        if(cn == null)
                cn.connectSQL();
            String sql = "SELECT MaSP,TenSP,Dongia,L.Maloai FROM Sanpham S, LoaiSP L where L.Maloai=S.Maloai and TenSp like N'%" + ten + "%'";
        return cn.LoadData(sql);
        }
